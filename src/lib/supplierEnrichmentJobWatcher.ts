  ...
  if (seen != null) {
    ...
    if (done.length === 1) {
      addNotification({
        ...
        body: 'Контакты собраны с сайта — проверьте карточку на странице «Поставщики».',
      });
    } else if (done.length > 1) {
      addNotification({
        title: 'Поставщики готовы к верификации',
        body: `У ${done.length} поставщиков собраны контакты — проверьте их на странице «Поставщики».`,
      });
    }
