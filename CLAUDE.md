...
**Сколько идёт билд и как за ним следить (2026-09-12).** Норма прод-билда —
~1.5 минуты. ~10 минут = полный пререндер всех ~285 путей: законно только когда
сборку заказало сохранение объекта в админке (`deploy_debounce`) или ручной
`PRERENDER_FORCE_FULL=1`; в остальных случаях это баг, идти в Build Logs.
