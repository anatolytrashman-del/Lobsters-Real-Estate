...
`scripts/prerender-snapshot.mjs`). ~10 минут = полный пререндер всех ~285 путей:
законно только когда сборку заказало сохранение объекта в админке
(`deploy_debounce`), изменился код публичных страниц (отпечаток
`public-build-id`) или ручной `PRERENDER_FORCE_FULL=1`; в остальных случаях это
баг — в Build Logs искать строку `[prerender] ИТОГ быстрого режима`: число «из-за
непригодной копии» в норме 0, ниже неё напечатаны причины.
